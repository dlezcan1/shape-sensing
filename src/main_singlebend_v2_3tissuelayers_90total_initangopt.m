%% main_singlebend_v2_3tissuelayers_90total_initangopt.m
%
% old comment: use needleshape_fmincon_v2.m 
...
%% load data
% New data on 02-17-2017 three-layered phantom [1/m] (90 mm insertion, in tissue)
tot_num_exp = 10;

Data_raw_tot = zeros(6,20,tot_num_exp);
for i_exp = 1:tot_num_exp
    sheet_num = ['T',num2str(i_exp)];
%     data_tmp = xlsread('changeframeResult_Insertion_0217_threelayerbeef.xlsx',sheet_num,'C:V');
    data_tmp = readmatrix('changeframeResult_Insertion_0217_threelayerbeef.xlsx','Sheet',sheet_num,'Range','C:V');
...
kc_meat_jig_mat = zeros(tot_num_exp,1);
kc_meat_beam_mat = zeros(tot_num_exp,1);
...
for ii = 1:tot_num_exp
    ...
    kc_m_jig = kc1_jig*0.5;%10;
    
    % initial guess of initial value eta
    eta = [kc1_jig*1.2;0;0;kc_m_jig];

    % initial cost function value
%     [wv,pmat,Rmat] = fn_intgEP_v1_2layers_kconst(eta,kc1_jig,kc2_jig,z_crit,theta0,ds,N,B,Binv);
    scalef0 = 1;
    Cval = costfn_opt_v2_singlebend_3layers(eta,kc1_jig,...
        data_jig,s_m,0,ds,theta0,z1_crit,z2_crit,N,B,Binv,scalef0);
    scalef = 1/Cval;
    ...
    UB = [0.005*ones(3,1);0.02]; % upper bound

    [x,fval,exitflag] = fmincon(@(x) costfn_opt_v2_singlebend_3layers(x,...
        kc1_jig,data_jig,s_m,0,ds,theta0,z1_crit,z2_crit,N,B,Binv,scalef),...
        x0,[],[],[],[],LB,UB,[],options);
    ...
    disp(['%>> w_init = '])
    disp(num2str(x))
    disp(['%>> kc_meat = ',num2str(x(4))])

    [wv_f,pmat,Rmat] = fn_intgEP_v1_3layers(x(1:3),kc1_jig,x(4),kc1_jig,z1_crit,z2_crit,...
        theta0,0,ds,N,B,Binv);
    
    % save
    wmat_tot_jig(:,:,ii) = wv_f;
    kc_meat_jig_mat(ii) = x(4);
    ...
    % parameters on intrinsic curvature (initial values)
    kc_m_beam = kc1_beam*0.5;
    
    % initial guess of initial value eta
    eta = [kc1_beam*1.2;0;0;kc_m_beam];
    
    % initial cost function value
%     [wv,pmat,Rmat] = fn_intgEP_v1_2layers(eta,kc1_beam,kc2_beam,z_crit,theta0,0,ds,N,B,Binv);
    scalef0 = 1;
    Cval = costfn_opt_v2_singlebend_3layers(eta,kc1_beam,...
        data_beam,s_m,0,ds,theta0,z1_crit,z2_crit,N,B,Binv,scalef0);
    scalef = 1/Cval;
    ...
    [x,fval,exitflag] = fmincon(@(x) costfn_opt_v2_singlebend_3layers(x,...
        kc1_beam,data_beam,s_m,0,ds,theta0,z1_crit,z2_crit,N,B,Binv,scalef),...
        x0,[],[],[],[],LB,UB,[],options);
    ...
    [wv_f,pmat,Rmat] = fn_intgEP_v1_3layers(x(1:3),kc1_beam,x(4),kc1_beam,z1_crit,z2_crit,...
        theta0,0,ds,N,B,Binv);

    % save
    wmat_tot_beam(:,:,ii) = wv_f;
    kc_meat_beam_mat(ii) = x(4);
...
for jj = 1:tot_num_exp
    ...
    pos_img = xlsread('T0217.xlsx',sheet_num,'C:D');
    if sign(pos_img(end)) == -1
        pos_img_tot{jj} = pos_img;
    elseif sign(pos_img(end)) == 1
        pos_img_tot{jj} = [pos_img(:,1),-pos_img(:,2)];
    end
end
...
%% plot needle deformation for paper (IEEE Sensor)
figure;
for i_plot = 1:4
    subplot(2,2,i_plot)
...
%             xlabel('Arclength [mm]')
%         end
%     elseif i_plot == 2 || i_plot == 4 || i_plot == 6
%         subplot(3,2,i_plot)
%         plot(s,wmat_tot_jig(2,:,i_plot),'k-','LineWidth',2)
%         hold on
%         plot(s,wmat_tot_beam(2,:,i_plot),'b-','LineWidth',2)
...
%% for presentation in IEEE Sensors
idx_plot = [1,2,3,5];

figure;
for ii = 1:2*length(idx_plot)
    if mod(ii,2) == 1
        i_plot = idx_plot((ii+1)/2);
        subplot(4,2,ii)
        plot(squeeze(pmat_tot_jig(3,:,i_plot)),squeeze(pmat_tot_jig(2,:,i_plot)),'k-','LineWidth',2)
        hold on
        plot(squeeze(pmat_tot_beam(3,:,i_plot)),squeeze(pmat_tot_beam(2,:,i_plot)),'b-','LineWidth',2)
    ...
    elseif mod(ii,2) == 0
        i_plot = idx_plot(ii/2);
        subplot(4,2,ii)
        plot([0 90],[0 0],'r-','LineWidth',2)
        hold on
        plot(squeeze(pmat_tot_jig(3,:,i_plot)),squeeze(pmat_tot_jig(1,:,i_plot)),'k-','LineWidth',2)
        plot(squeeze(pmat_tot_beam(3,:,i_plot)),squeeze(pmat_tot_beam(1,:,i_plot)),'b-','LineWidth',2)
        hold off
        ...
        if ii == 2
            legend('interpolation','model (jig)','model (beam)')
        end
    end
end
