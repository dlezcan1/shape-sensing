%% Shape_ideal_p_3layer_Dimitri.m
%
% used for the simulation of data-based single bend needle insertion.
%
% - written by Dimitri Lezcano

global p L lengths N ds theta0 B Binv w_init_ref s1_crit s2_crit N_arclengths z1_crit z2_crit

% saving set-up
save_bool = true;
directory = "Dimitri/Data/";
directory = directory + "Archive/3-Layer/TipAError_Cost_Ideal/";
file_save = directory + "kc_3layer_sms_p-%.4f_ideal";
...
title(sprintf('3-D double-layer insertion: p = %.5f', p))
legend('FontSize',16);
grid on; axis equal;

f2 = figure(2);
set(gcf,'units', 'normalized', 'position', [1/8, 1/8, 6/8, 6/8]);
sgtitle(sprintf('2-D double-layer insertion: p = %.5f', p))
...
function cost = cost_fn(p, kc1, kc2, kc3)    
