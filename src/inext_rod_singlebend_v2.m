...
%
% To compare kappa_c value of different insertion lengths, i.e., to check
% the dependence of kappa_c on the insertion length L.
%
% conclusion: maintaining the same needle shape means that the forces
...
press = 5e-3; %2e-2; %1e-2;%5e-3;
fric = 1e-3; %2e-3; %1e-3;%1e-3;%1e-3;%5e-3;%1e-3;
...
kc = 0.05*press*L1^2/2/BendStiff;
pr = 0.1*press*L1;
fr = 0.1*fric*L1;
...
% w(0) = [kc;0;0] and f(0) = [0;pr;fr]
% start with the results from the beam theory
kc = 0.05*press*L2^2/2/BendStiff;
pr = 0.1*press*L2;
fr = 0.1*fric*L2;
...
% w(0) = [kc;0;0] and f(0) = [0;pr;fr]
% start with the results from the beam theory
kc = 0.05*press*L3^2/2/BendStiff;
pr = 0.1*press*L3;
fr = 0.1*fric*L3;
