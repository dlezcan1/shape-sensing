%% main_needle_doublebend_homlayer_theta0opt.m
%
...
ds = 1;%0.5; % in mm (0.5 for Euler integration and 1 for R-K method)
L = 90;%80; % in mm
s = [0:ds:L];
N = length(s);
...
for ii = 1:tot_num_exp
    ...
    % initial cost value
    scalef0 = 1;
    Cval = costfn_opt_needle_doublebend_homlayer(eta,data_jig,s_m,ix_s_turn,ds,N,B,Binv,scalef0);
    scalef = 1/Cval;
...
%         'MaxFunEvals',10000,'Display','iter','MaxIter',maxiter,'MaxFunEvals',100000);
    ...
    [x,fval,exitflag] = fmincon(@(x) costfn_opt_needle_doublebend_homlayer(x,data_jig,s_m,ix_s_turn,...
...
%     exitflag
    ...
    kc_f1 = kc_f*((s1(end) - s1(1))/L)^(2/3);
    kc_f2 = kc_f*((s2(end) - s2(1))/L)^(2/3);
    
    k0_1_f = kc_f1*(1 - s1/L).^2;
%     k0_1_f = kc_f1*(s1(end) - s1).^2/L^2;
    k0_2_f = -kc_f2*(1 - s2/L).^2;
%     k0_1_f = kc_f1/L^2*(s1(end) - s1).^2;
% %     k0_1_f = kc_f1*(1 - s1/s1(end)).^2;
%     k0_2_f = -kc_f2*(s2 - s1(end)).^2/L^2.*(1 - s2/L);
% %     k0_2_f = -kc_f2*(1 - s2/s1(end)).^2.*(1 - s2/L);
    k0_turn = (k0_1_f(end) + k0_2_f(1))/2; %0;
    k0_f = [k0_1_f(1:end-1),k0_turn,k0_2_f(2:end)];

    k0prime1_f = -2*kc_f1/L*(1 - s1/L);
%     k0prime1_f = -2*kc_f1/L^2*(s1(end) - s1);
    k0prime2_f = 2*kc_f2/L*(1 - s2/L);
%     k0prime1_f = -2*kc_f1/L^2*(s1(end) - s1);
% %     k0prime1_f = -2*kc_f1/s1(end)*(1 - s1);
%     k0prime2_f = -2*kc_f2/L^2.*(s2 - s1(end)).*(1 - s2/L) + kc_f2/L^3*(s2 - s1(end)).^2;
% %     k0prime2_f = 2*kc_f2/s1(end)*(1 - s2/s1(end)).*(1 - s2/L) + kc_f2/L*(1 - s2/s1(end)).^2;
    k0prime_f_peak = (k0_2_f(2) - k0_1_f(end-1))/2/ds;
    k0prime_f = [k0prime1_f(1:end-1),k0prime_f_peak,k0prime2_f(2:end)];
...
%     pmat_tot_jig(:,:,ii) = pmat;
    ...
    % initial cost value
    scalef0 = 1;
    Cval = costfn_opt_needle_doublebend_homlayer(eta,data_beam,s_m,ix_s_turn,ds,N,B,Binv,scalef0);
    scalef = 1/Cval;
    ...
    [x,fval,exitflag] = fmincon(@(x) costfn_opt_needle_doublebend_homlayer(x,data_beam,s_m,ix_s_turn,...
        ds,N,B,Binv,scalef),x0,[],[],[],[],LB,UB,[],options);
...
for jj = 1:tot_num_exp
    jj_exp = dbexpix(jj);
    if jj_exp <= 9
        sheet_num = ['0',num2str(jj_exp)];
    elseif jj_exp >= 10
        sheet_num = [num2str(jj_exp)];
    end
    pos_img = xlsread('T0425_2017_v1.xlsx',sheet_num,'C:D');
...
%     pos_img = pos_img_tot{ii};
    if mod(ii,2) == 0
        pos_img = [pos_img_tot{ii}(:,1),-pos_img_tot{ii}(:,2)];
    else
        pos_img = pos_img_tot{ii};
    end
...
%     pos_img_org = pos_img_tot{jj};
    if mod(jj,2) == 0
        pos_img_org = [pos_img_tot{jj}(:,1),-pos_img_tot{jj}(:,2)];
        %pos_img_org = -pos_img_tot{jj};
    else
        pos_img_org = pos_img_tot{jj};
    end
