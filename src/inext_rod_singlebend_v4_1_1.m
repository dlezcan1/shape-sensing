...
%
% This is for double-layer single bend needle insertion.
%
% inherited from inext_rod_sionglebend_v4.m 
% difference: for distributed loads in v4, we use a single parameter.
% here v4_1, use two parameters.
%
%
% Conclusion: 
%
...
press1_1 = 1e-2;%5e-3; %1e-2;%5e-3;
fric1_1 = 3e-3;%1e-3; %1e-3; %1e-3;%1e-3;%1e-3;%5e-3;%1e-3;
press1_2 = 2e-2;%1e-2;
fric1_2 = 6e-3;%2.5e-3;
...
% plot(s1,y1)
% legend('\omega_1 (s)',...
%     '\kappa_{c1}((s*-s)/L)^2+\kappa_{c2}(1-s*/L)(1+s*/L-2s/L)')
% grid on
% xlabel('s [mm]')
% ylabel('\omega_1')
% title('L = 60 mm')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   plot the configurations of the rod
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% configurations
figure;
...
po1 = polyfit(log(Lv),log(kc1vec),1) % determine p value in kc1
po2 = polyfit(log(Lv),log(kc2vec),1) % determine p value in kc2

figure;
plot(Lv, kc1vec, 'ko-')
xlabel('insertion length [mm]')
ylabel('\kappa_{c,1} [1/mm]')

figure;
plot(Lv, kc2vec, 'ko-')
xlabel('insertion length [mm]')
ylabel('\kappa_{c,2} [1/mm]')
...
total_er = TipAerror(pmat_total);

disp(['>>> error = ',num2str(total_er),', p_1 = ',num2str(-po1(1)),...
    ', p_2 = ',num2str(-po2(1))])


% %% save for plots in the paper
% save('pmat_inextrod_2layer','pmat_total');
% 
% wmat_total = cell(1,4);
% wmat_total{1} = {s1, wv1, y1};
% wmat_total{2} = {s2, wv2, y2};
% wmat_total{3} = {s3, wv3, y3};
% wmat_total{4} = {s4, wv4, y4};
% 
% save('wmat_inextrod_2layer','wmat_total')
