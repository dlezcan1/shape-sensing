...
function [pos, wv, Rmat, kc, w_init,theta0,LB,UB] = singlebend_needleshape(curvatures, aa_tip_locs, ...
...
%   - theta0: the offset insertion angle (double, default is 0)
    ...
    LB = [-0.01*ones(3,1);0.0; deg2rad(-theta_max)]; % lower bound
    UB = [0.01*ones(3,1); 0.01;  deg2rad(theta_max)]; % upper bound
    ...
    [x, fval, exitflag] = fmincon( @(x) costfn_shape_singlebend_theta0(x, curvs_aa,...
...
% cost function for needle shape w/ theta0 optimization
function y = costfn_shape_singlebend_theta0(eta,data,s_index_meas,ds,N,B,Binv,scalef,weights) 
    weights = weights/sum(weights, 'all');
    % unpack the variables
    w_init = eta(1:3); 
    kc = eta(4); 
    ...
    % integration of the E-P equation 
    wv = fn_intgEP_w0_Dimitri(w_init, w0, w0prime,theta0,0,ds,N,B,Binv);
