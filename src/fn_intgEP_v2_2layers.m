function [wv,pmat,Rmat] = fn_intgEP_v2_2layers(w_init,kc1r,kc2r,z_crit,theta0,s0,ds,N,B,Binv)
%
...
global p % needed for the simulation regarding p

L = s0 + (N-1)*ds; % in mm
s = [s0:ds:L];

% determine s_crit (90 mm)
[w1,p1] = fn_intgEP_v1_1layer(w_init,kc1r,theta0,s0,ds,N,B,Binv);
...
Lr = 90; % reference insertion length
% kc1 = kc1r*(Lr/s_crit)^p; % polynomial dependence of kappa_c 
% kc2 = kc2r*(Lr/(L-s_crit))^p; % polynomial dependence of kappa_c
kc1 = kc1r*(Lr/L)^p; % polynomial dependence of kappa_c 
kc2 = kc2r*(Lr/L)^p; % polynomial dependence of kappa_c
