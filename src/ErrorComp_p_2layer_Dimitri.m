...
kc_s = 0.002;%0.0017; % for first tissue 
...
for i = 1:Np
    ...
    w_init = [kc1;0;0];
    [wv1,pmat1,Rmat1] = fn_intgEP_v3_2layers(w_init,kc_s_m,kc_h_m,z_crit,0,0,ds,N1,B,Binv);
    ...
    w_init = [kc2;0;0];    
    [wv2,pmat2,Rmat2] = fn_intgEP_v3_2layers(w_init,kc_s_m,kc_h_m,z_crit,0,0,ds,N2,B,Binv);
    ...
    w_init = [kc3;0;0];
    [wv3,pmat3,Rmat3] = fn_intgEP_v3_2layers(w_init,kc_s_m,kc_h_m,z_crit,0,0,ds,N3,B,Binv);
    ...
    w_init = [kc4;0;0];
    [wv4,pmat4,Rmat4] = fn_intgEP_v3_2layers(w_init,kc_s_m,kc_h_m,z_crit,0,0,ds,N4,B,Binv);
    ...
    w_init = [kc5;0;0];
    [wv5,pmat5,Rmat5] = fn_intgEP_v3_2layers(w_init,kc_s_m,kc_h_m,z_crit,0,0,ds,N5,B,Binv);
    ...
    total_error(i) = TipAerror_Dimitri(pmat_total, lengths);
end

data = [data; kc_h*ones(size(pv')), pv', total_error'];
disp(data(end,:));
disp(' ');
...
surf(X, Y, Z);
colorbar()
xlabel('\kappa_{c,2}'); ylabel('p'); zlabel('error [mm^2]')
title('double-layer insertion: \kappa_{c,2} vs p')
grid on
view([0,90])



Z_inverted = max(Z,[],'all') - Z;
% figure;
subplot(2,1,2);
surf(X, Y, Z_inverted);
colorbar()
xlabel('\kappa_{c,2}'); ylabel('p'); zlabel('max error - error [mm^2]')
title('Inverted | double-layer insertion: \kappa_{c,2} vs p')
grid on
view([0,90])

sgtitle("\kappa_{c,1} = " + sprintf("%.4f",kc_s));

%% Save the Results
file_name = "Dimitri/Data/" + sprintf("kc-s_%.4f_pwritematrix_error",kc_s);

writematrix(data, file_name + '.csv')
