...
f2 = figure(2);
set(gcf,'units', 'normalized', 'position', [1/10, 1/10, 6/8, 6/8]);%[1/8, 1/8, 6/8, 6/8]);
...
%title('x-z axis view');
hl1 = legend('FontSize', 16); %,'location','northeastoutside');
grid on; axis equal;

hl1.Position(1) = SS1.Position(3) + 0.1; %--> then in the plot, move the legend box

% add second plot (error)
...
%xline(z_crit, 'r--', 'Tissue Boundary','DisplayName','Tissue Boundary');
xlabel('s [mm]'); ylabel('Avg. Dev. Error [mm]');
