function [p, cost] = kappa_c_analysis_fn(kc_init, save)
%% Function for running the kappa_c vs L analysis
...
cost_fn = @(p) cost_fn_kc_analysis(B, Binv, kc_init, w_init, L_init, lengths, p);  % area metric
% cost_fn = @(p) cost_fn_kc_analysis_v2(B, Binv, kc_init, w_init, L_init, ...
% lengths, p); % deviation metric 
% p0 = 2/3;
...
for i = 1:length(positions)
    positions{i} = position_kcp(B, Binv, kc_init, L_init, lengths(i), p);
...
for i = length(positions):-1:1
    p_i = positions{i};
    plot3(p_i(3,:),p_i(1,:),p_i(2,:),'Displayname',sprintf("L = %d mm", lengths(i))); hold on;
...
for i = length(positions):-1:1
    p_i = positions{i};
    plot(p_i(3,:),p_i(2,:),'Displayname',sprintf("L = %d mm", lengths(i))); hold on;
